...
/**
 * Created by liruibao on 2017/9/5.
 */

public class FootBallView extends View {
    private static final int playerNum = 5; // 球员的数量
    private static final float[] playerPosY = {1/8,2/8,3/8,5/8,6/8,7/8};
    private static final float[] playerPosX = {1/3,1/4};
...
//                Toast.makeText(mContext, mBall.detect(goalBlue) ? "红队胜利" : "蓝队胜利", Toast.LENGTH_LONG).show();
    ...
    private void initPlayer(Canvas canvas) {
        ...
        for (int i = 0; i < playerNum; i++) {
            mPlayerRed[i].radius = realHeight / 20;
            Paint paint = new Paint();
        ...
        for (int i = 0; i < playerNum; i++) {
            mPlayerBlue[i].radius = realHeight / 20;
            Paint paint = new Paint();
...
//            }
    ...
    private void touchDetect(float x, float y) {
